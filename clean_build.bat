@echo off
:: Clean build script for DemoTape using CMake on Windows
:: This script deletes the build directory before rebuilding
...
if %ERRORLEVEL% NEQ 0 (
  ...
  echo.
  echo === Using --trace to see more details: ===
  echo Running cmake with --trace flag to see more verbose output...
  cmake --trace-expand "%SOURCE_DIR%" > logs\cmake_trace.log 2>&1
)
